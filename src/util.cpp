...
const char * const BITCOIN_CONF_FILENAME = "sphere.conf";
const char * const BITCOIN_PID_FILENAME = "sphered.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "sphere" is a composite category enabling all Sphere-related debug output
            if(ptrCategory->count(string("sphere"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\SphereCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\SphereCore
    // Mac: ~/Library/Application Support/SphereCore
    // Unix: ~/.spherecore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "SphereCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/SphereCore";
#else
    // Unix
    return pathRet / ".spherecore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty sphere.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override sphere.conf
